...
using Ionic.Zip;
using PS4_Compiler_GUI_with_VS_EXC;
    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                if (args.Length > 0)
                {
                    switch (args[0].Trim().ToLower())
                    {
                        case "--install_silent":
                            InstallUpdate();
                   MessageBox.Show("Compiler Updated!", $"Your t7 compiler installation was just updated. You may proceed with your compilation action.", MessageBoxButtons.OK);
                            return;
            ...
            catch (Exception e)
            {
                if (IsUpdating)
                {
                MessageBox.Show("Error Updating!", $"Failed to install: {e}", MessageBoxButtons.OK);
                }
                else
                {
        ...
        static void DeployCompiler(string compilerDirectory, string defaultProjectDirectory, string solutionDirectory)
        {
            ...
            System.IO.Compression.ZipFile.CreateFromDirectory(build, Path.Combine(depot, "update.zip"));
        ...
        public static void FetchUpdateContents()
        {
            ...
            System.IO.Compression.ZipFile.ExtractToDirectory(UpdateTempFilename, UpdateTempDirname);
        }
